...
public class UserDetailsServiceImp implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String userid) throws UsernameNotFoundException {
        // LoginRepository를 통해 userid로 LoginEntity를 조회합
        ...
        // 이 객체를 Spring Security가 사용하여 인증 및 권한 부여를 처리
        return createUser(loginEntity);
    }

    private UserDetails createUser(LoginEntity loginEntity) {
        // 사용자의 Role 정보를 GrantedAuthority 객체로 변환
        // Spring Security는 역할(Role) 앞에 "ROLE_" 접두사가 붙는 것을 기본으로 함
